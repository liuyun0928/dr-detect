...
def readTrainData(trainDir):
    ...
    for root, dirs, files in images:
        for imageFileName in files:
            ...
            imageFullPath = os.path.join(root, imageFileName)
            #print(imageFullPath)
...
sys.stdout.flush()
readTrainData(os.path.join(os.path.dirname(os.path.abspath(__file__)), "data/colored_images"))
...
import csv
def readTrainCsv():
    raw_df = pd.read_csv(os.path.join(os.path.dirname(os.path.abspath(__file__)), "data/trainLabels.csv"), sep=',')
...
print("converted hash to dataframe")
...
trainY =  np_utils.to_categorical(trainY, num_classes=NUM_CLASSES)
valY =  np_utils.to_categorical(valY, num_classes=NUM_CLASSES)
